...
كل جامع كان يحمل نسخته من `_parse_date` بصيغة مختلفة (أربع نسخ). هنا مصدر
واحد لكل الصيغ التي نستقبلها، وكلها تعيد datetime **واعياً بالمنطقة الزمنية**
(UTC) — الخلط بين الواعي وغير الواعي كان يسبب مقارنات فاشلة في طبقة API.
"""
