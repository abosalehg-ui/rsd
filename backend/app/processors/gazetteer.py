...
KSA_REFERENCE_POINTS: tuple[tuple[str, float, float], ...] = (
    ("الرياض", 24.713, 46.675), ("الدمام", 26.434, 50.103), ("الجبيل", 27.011, 49.658),
    ("رأس تنورة", 26.644, 50.159), ("الخفجي", 28.439, 48.491), ("حفر الباطن", 28.432, 45.971),
    ("عرعر", 30.975, 41.038), ("القريات", 31.332, 37.343), ("تبوك", 28.383, 36.566),
    ("حقل", 29.289, 34.938), ("ينبع", 24.089, 38.064), ("جدة", 21.485, 39.193),
    ("جازان", 16.889, 42.570), ("نجران", 17.493, 44.128), ("شرورة", 17.469, 47.107),
    ("الأحساء", 25.383, 49.587), ("سلوى", 24.735, 50.829), ("البطحاء", 24.100, 51.550),
    ("الوديعة", 17.050, 47.100), ("طريف", 31.672, 38.663), ("الجوف", 29.970, 40.200),
    ("تيماء", 27.630, 38.550), ("المدينة المنورة", 24.468, 39.614), ("أبها", 18.216, 42.505),
    ("وادي الدواسر", 20.460, 44.800), ("الخرخير", 18.870, 51.130), ("شيبة", 22.520, 54.000),
)
...
def nearest_ksa_point(lat: float | None, lon: float | None) -> tuple[float, str] | None:
    ...
    if lat is None or lon is None:
        return None
    best = min(
        ((haversine_km(lat, lon, plat, plon), name) for name, plat, plon in KSA_REFERENCE_POINTS),
        key=lambda x: x[0],
    )
