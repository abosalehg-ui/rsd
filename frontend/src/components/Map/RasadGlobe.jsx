...
function sizeFor(severity) {
  return severity === 'critical' ? 0.55 : severity === 'high' ? 0.38 : severity === 'medium' ? 0.25 : 0.16;
}
...
export default function RasadGlobe({
  ...
  // النقاط (أحداث + إيران + مفاعلات + قواعد)
  useEffect(() => {
      ...
      events.forEach(ev => {
        ...
        points.push({
          ...
          label: `<div style="direction:rtl;font-family:Tajawal,sans-serif;background:#111827;border:1px solid #1e293b;padding:6px 8px;border-radius:6px;max-width:280px">
            <div style="color:${eventColor(ev)};font-size:11px;font-weight:700;margin-bottom:2px">${(CATEGORIES[ev.category]||CATEGORIES.general).icon} ${ev.title || ''}</div>
