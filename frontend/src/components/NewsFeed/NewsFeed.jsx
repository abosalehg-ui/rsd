  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[#1e293b]">
        ...
        <button onClick={() => setShowFilters(!showFilters)} className="p-1 rounded hover:bg-[#1e293b] text-slate-400" aria-label={t('news.title')} aria-expanded={showFilters}>
      ...
      <div className="flex-1 overflow-y-auto">
        {events.length === 0 ? (
          <div className="flex items-center justify-center h-32 text-sm text-slate-500">{t('news.noEvents')}</div>
        ) : (
          events.map(event => {
            ...
            return (
              <div
                ...
                className={`border-b border-[#1e293b]/50 px-3 py-2 cursor-pointer transition-colors hover:bg-[#1e293b]/30 focus:outline-none focus:ring-1 focus:ring-cyan-500 ${isExpanded ? 'bg-[#1e293b]/20' : ''}`}
                  ...
                  <div className="mt-2 pt-2 border-t border-[#1e293b]/50">
                    ...
                    {safeUrl(event.url) && (
                      <a href={safeUrl(event.url)} target="_blank" rel="noopener noreferrer"
                        ...
                        <ExternalLink className="w-3 h-3" /> {t('news.sourceLink')}
                      </a>
                    )}
                  </div>
                )}
